...
export class Menu extends ClassifiedElement {
    ...
    onKeyDown(event) {
        ...
        if (code === 'Escape') {
            ...
            if (!this.open && this.focused)
                this.onSelection(this.focused);
        }
        else if (code === 'ArrowDown' || code === 'ArrowRight') {
            event.preventDefault();
            if (!this.focused)
                this.focused = this.options[0]?.value;
            else {
                const idx = this.options.findIndex(({ value }, _idx) => value === this.focused);
                if (idx > -1 && idx < this.options.length - 1)
                    this.focused = this.options[idx + 1].value;
            ...
            event.preventDefault();
            if (!this.focused)
                this.focused = this.options[this.options.length - 1]?.value;
            else {
                const idx = this.options.findIndex(({ value }, _idx) => value === this.focused);
                if (idx > 0)
                    this.focused = this.options[idx - 1].value;
    ...
    get listElement() {
        ...
        return html `<ul class=${classMap(classes)} role="menu">
            ${repeat(this.options, ({ label }) => label, ({ label, value, classes }) => html `<li
