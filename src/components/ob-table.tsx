...
export const ObTable = ({ data, tableClassName, fillWidth = true }: ObTableProps) => {
    ...
    const rows = useMemo(() => (data.length > 0 ? data.map((d) => Object.values(d)) : []), [])
    ...
    const THEAD_TR = useCallback(
        ({ children }: PropsWithChildren) => <tr className="bg-white/80 border-b-neutral-200 backdrop-blur-sm">{children}</tr>,
        []
    )
            ...
            return (
                <th
                    ...
                    className={classNames(
                        className,
                        'border-b',
                        cellSeparators.value && !withResizer ? 'border-r shadow-sm' : null,
                        'relative whitespace-nowrap p-1.5'
                    )}
                >
                    {children}
                    {withResizer && <Resizer height={`${height}px`} columnRef={ref} />}
                </th>
            )
        },
        [height]
    ...
    const TR = useCallback(({ children }: PropsWithChildren) => <tr className="hover:bg-neutral-300/10">{children}</tr>, [])
    ...
    const TD = useCallback(
        ({
            ...
            >
                ...
                <span className={selectableText.value ? 'select-text' : undefined}>{children}</span>
            </td>
        ),
        []
    )

    const Resizer = ({ height, columnRef }: { height: string; columnRef: Ref<HTMLTableCellElement> | null }) => {
        const xPositionSignal = signal(0)
        const widthSignal = signal(0)

        const onMouseDown: h.JSX.MouseEventHandler<HTMLDivElement> = useCallback(({ clientX }) => {
            ...
            widthSignal.value = parseInt(window.getComputedStyle(column).width, 10)
    ...
    return (
        <table
            ref={ref}
            className={classNames(tableClassName, fillWidth ? 'w-full' : null, 'select-none border-separate border-spacing-0 text-left')}
        >
            <THEAD>
                <THEAD_TR>
                    {columns.map((k, idx) => (
                ...
                {rows.map((row, idx) => (
                    <TR>
                        {row.map((value) => (
                            ...
                            <TD includeBottomBorder={rows.length - 1 !== idx}>{Array.isArray(value) ? value.join(', ') : value}</TD>
                        ))}
                    </TR>
                ))}
            </tbody>
